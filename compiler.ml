...
module Reader : READER = struct
  ...
  (* need to check, done by us *)
  and nt_paired_comment str = 
    let nt_open = char '{' in
    let nt_close = char '}' in
    let nt_text = disj_list[unitify(nt_string); unitify(nt_char); unitify((diff nt_any (disj nt_open nt_close)))] in 
    let nt_nest = star(disj nt_text nt_paired_comment) in 
    let nt_final = unitify(caten nt_open (caten nt_nest nt_close)) in 
    nt_final str

  (* need to check, done by us , check if we need the unitify in lines 80, 82*)
  and nt_sexpr_comment str = 
    let nt1 = word "#;" in
    let nt2 = unitify (nt_sexpr) in 
    let nt3 = caten nt_sexpr_comment nt2 in
    let nt3 = unitify nt3 in 
    let nt4 = disj nt3 nt2 in 
    let nt4 = unitify nt4 in 
    let nt1 = unitify (caten nt1 nt4) in 
    nt1 str  
  and nt_comment str =
    disj_list
      [nt_line_comment;
  ...
  and nt_digit str =
    ...
    let digit= pack digit (fun (ch) -> int_of_char ch - zero) in 
    digit str 

  and nt_hex_digit str =
    let nt_hex1 = range 'a' 'f' in 
    let nt_hex1= pack nt_hex1 (fun (ch) -> int_of_char ch - 87) in
    let nt_hex2 = range 'A' 'F' in 
    let nt_hex2= pack nt_hex2 (fun (ch) -> int_of_char ch - 55) in
    let nt_hex2 = disj nt_hex1 nt_hex2 in
    let nt_final = disj nt_digit nt_hex2 in
    nt_final str

  and nt_nat str =
    let nt_final=plus (range '0' '9') in
    let nt_final=pack nt_final (fun(dig)->int_of_string(string_of_list dig)) in 
    nt_final str 
  and nt_hex_nat str = 
  ...
  and nt_optional_sign str =
    let maybify nt none_value= pack (maybe nt)(function 
    | None-> none_value
    |Some x -> x) in 
    let nt_plus= pack (char '+') (fun _ -> true) in 
    let nt_minus= pack (char '-') (fun _ -> false) in 
    let nt_final= maybify(disj nt_plus nt_minus) true in 
  nt_final str
  ...
  and nt_float_a str = 
    ...
    let nt_final = caten nt_integer_part nt_point in 
    let nt_man_exp = caten (maybe nt_mantissa) (maybe nt_exponent) in 
    let nt_final = caten nt_final nt_man_exp in 
    let nt_final = pack nt_final (fun (((num, point),(man, exp)))->match(man, exp) with
                                                  |(Some(man), Some(exp))->num*.(exp)+.(man *. (exp))
                                                  |(Some(man), None)->( num)+. man
                                                  |(None, Some(exp))->( num)*.(exp)
                                                  |(None, None)->( num)) in 
    nt_final str

  and nt_float_b str =   
    let nt_point = char '.' in  
    let nt_final = caten nt_mantissa (maybe nt_exponent) in
    let nt_final = caten nt_point nt_final in 
    let nt_final =pack nt_final (fun (point,(man, exp))->match exp with
                                          | Some(exp)-> man *.exp
                                          | None-> man) in 
    nt_final str  
    
  and nt_float_c str =
    let nt_final = caten nt_integer_part nt_exponent in 
    let nt_final =pack nt_final (fun (num, exp)-> ( num)*.exp) in 
    nt_final str

  and nt_float str =
    let nt_final= caten (maybe nt_optional_sign) (disj_list[nt_float_a; nt_float_b; nt_float_c]) in 
    let nt_final = pack nt_final (fun (sign, num) -> match sign with
                                                    | Some(sign) -> if (sign== false) then num*.(-1.) else num
                                                    | None -> num) in
                                                    let nt_final =pack nt_final (fun (num)->ScmReal(num)) in 
    nt_final str
  ...
  and make_char_named name ch = 
     let nt_final=word_ci name in 
     pack nt_final (fun _->ch)                 

  and nt_char_named str =
    let nt_final=disj_list[(make_char_named "newline" '\n'); (make_char_named "page" '\012'); (make_char_named "return" '\r');
                          (make_char_named "space" ' '); (make_char_named "tab" '\t'); (make_char_named "nul" (Char.chr 0));] in 
    nt_final str 
  ...
  and nt_symbol str =
    let nt_final = plus (nt_symbol_char) in 
    let nt_final = pack nt_final string_of_list in
    let nt_final = pack nt_final (fun name -> ScmSymbol (String.lowercase_ascii name)) in
    let nt_final = diff nt_final nt_number in
    nt_final str 
  ...
  and nt_string_part_dynamic str =
    let nt_open = word "~{" in 
    let nt_close =char '}' in 
    let nt_inside = nt_sexpr in 
    let nt_final = caten nt_open (caten nt_inside nt_close) in
    let nt_final = pack nt_final (fun (op, (sexpr, close))->Dynamic(ScmPair(ScmSymbol("format"),ScmPair(ScmString("~a"), ScmPair(sexpr,ScmNil))))) in 
    nt_final str 
  ...
  and nt_vector str =
    let nt_open = word "#(" in
    let nt_close = caten nt_skip_star (char ')') in
    let nt_close = pack nt_close (fun _ -> ScmVector []) in
    let nt_inside = plus nt_sexpr in
    let nt_close_again = char ')' in
    let nt_inside = caten nt_inside nt_close_again in
    let nt_inside = pack nt_inside (fun (sexprs, _) -> ScmVector sexprs) in
    let nt_close = disj nt_close nt_inside in
    let nt_open = caten nt_open nt_close in
    let nt_open = pack nt_open (fun (_, sexpr) -> sexpr) in
    nt_open str                 
  ...
  nt1 str
    ...
    and make_nt_spaced_out nt_skip nt=
let nt1 = caten nt_skip (caten nt nt_skip) in
let nt1 = pack nt1 (fun (_,(e,_))->e) in
nt1
      and nt_list str = 
      let nt1 = char '(' in
      let nt2 = pack (make_nt_spaced_out nt_skip (char ')'))(fun _-> ScmNil) in
      let nt3 = plus nt_sexpr in
      let nt4 = pack (char ')')(fun _-> ScmNil) in
      let nt5 = pack (caten (char '.')(caten nt_sexpr(char ')')))(fun (_,(sexp,_))->sexp) in
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_and_clauses expr = function
    ...
    | expr' :: exprs -> 
        ...
        ScmPair(ScmSymbol"if", ScmPair(expr, ScmPair(dit,ScmPair(ScmBoolean false, ScmNil))));;
  ...
  let rec macro_expand_cond_ribs ribs =
    ...
    let rec scm_append scm_list sexpr =
      ...
      | _ -> raise (X_syntax("Append expects a proper list"));;

    let rec scm_list_to_list = function
    | ScmPair (hd, tl) -> hd::(scm_list_to_list tl)
    | ScmNil -> []
    | sexpr -> raise (X_syntax("Expected proper list"));;
  ...
  let rec scm_map f sexpr =
    ...
    | _ -> raise (X_syntax("Map expects a list"));;
...
let rec scm_zip f sexpr1 sexpr2 =
  ...
  | _, _ ->
      raise (X_syntax("Zip expects 2 lists of equal length"));;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "let*",
               ScmPair
                 (ScmPair
                  ...
                  exprs)) -> tag_parse(ScmPair(ScmPair(ScmSymbol("lambda"), ScmPair(ScmPair(var,ScmNil), exprs)),ScmPair(value,ScmNil)))
    | ScmPair (ScmSymbol "let*",
               ScmPair 
                (ScmPair
                 (ScmPair (var, ScmPair (arg, ScmNil)),ribs),
                        exprs)) ->
                          let rest= ScmPair(ScmSymbol "let*",ScmPair(ribs, exprs)) in
                          tag_parse(ScmPair(ScmSymbol "let", ScmPair(ScmPair(ScmPair(var,ScmPair(arg,ScmNil)),ScmNil),ScmPair(rest,ScmNil))))
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       handle_let_rec ribs exprs
    ...
    | sexpr -> raise (X_syntax
                       (Printf.sprintf
                          ...
                          and duplicate_exists = function
                          | [] -> false
                          | hd::tl -> List.mem hd tl || duplicate_exists tl 
                        and handle_let ribs body =
                          let vars_names_list = scm_map (fun (sexpr) -> match sexpr with 
                                                      | ScmPair(var, ScmPair(value, ScmNil)) -> var
                                                      | _ -> raise (X_syntax("Sexpr structure not recognized in let"))) ribs in
                          let vals_list = scm_map (fun (sexpr) -> match sexpr with 
                                                      | ScmPair(var, ScmPair(value, ScmNil)) -> value
                                                      | _ -> raise (X_syntax("Sexpr structure not recognized in let"))) ribs in
                          if (duplicate_exists (scm_list_to_list vars_names_list))
                          then raise (X_syntax("args names must be unique"))
                          else tag_parse(ScmPair(ScmPair(ScmSymbol("lambda"), ScmPair(vars_names_list, body)), vals_list))

                        and handle_let_rec ribs body =
                          let vars_names_scm_list = scm_map (fun (sexpr) -> match sexpr with 
                                                      | ScmPair(var, ScmPair(value, ScmNil)) -> var
                                                      | _ -> raise (X_syntax("Sexpr structure not recognized in letrec"))) ribs in
                          let vals_scm_list = scm_map (fun (sexpr) -> match sexpr with 
                                                      | ScmPair(var, ScmPair(value, ScmNil)) -> value
                                                      | _ -> raise (X_syntax("Sexpr structure not recognized in letrec"))) ribs in 
                          let modified_ribs = scm_map (fun (var) -> 
                              ScmPair(var, ScmPair(ScmPair((ScmSymbol "quote"), ScmPair(ScmSymbol ("whatever"), ScmNil)), ScmNil))) vars_names_scm_list in
                          let set_body = scm_zip (fun var value -> ScmPair(ScmSymbol("set!"), ScmPair(var, ScmPair(value, ScmNil)))) vars_names_scm_list vals_scm_list in
                          let modified_body = scm_append set_body body in
                          tag_parse(ScmPair(ScmSymbol("let"), ScmPair(modified_ribs, modified_body)));;           
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmSeq exprs -> ScmSeq'(List.map(fun (seq) -> run seq params env) exprs)
      | ScmOr exprs -> ScmOr'(List.map(fun (sOr) -> run sOr params env) exprs)
  ...
  let should_box_var name expr params = 
    let (al', bl')= find_reads_and_writes name expr params in
    let pros=cross_product al' bl' in 
    let prod=List.filter(fun((pr1,env1),(pr2,env2))->
      match pr1 with
        |Var' (_,Bound(major,_))->
          (match pr2 with
            |Var' (_, Param _)->true
            |Var' (_, Bound(major',_))->List.nth env1 major!= List.nth env2 major'
            |_->false)
        |Var' (_,Param _)->
          (match pr2 with
            |Var' (_,Free)->false
...
let remove_duplicates =
  fun list->
    let uniq xs x = if List.mem x xs then xs else x :: xs in
    List.rev (List.fold_left uniq [] list);;
let collect_constants =
  let rec run= function
    | ScmBox'(var) -> []
    | ScmBoxGet'(var) -> []
    | ScmBoxSet'(var, value) -> run value
    | ScmConst'(const) -> [const] 
    | ScmVarGet'(var) -> []
    | ScmIf'(test, dit, dif) -> 
      let test_pairs = run test in
      let dit_pairs = run dit in 
      let dif_pairs = run dif in
      (test_pairs@dit_pairs)@dif_pairs
    | ScmSeq'(expr_list) -> 
      runs expr_list
    | ScmVarSet'(var, value) -> 
      run value
    ...
    | ScmLambda' (_,_, body) -> 
      run body
    | ScmApplic'(app_name, app_args,_) -> 
      (run app_name)@
     runs app_args
    and runs exprs'= 
      List.fold_left(fun full expr' -> full @ (run expr')) [] exprs' 
  ...
  let search_constant_address = 
    let rec find sexpr table =
      match table with
      |[] -> raise (X_this_should_not_happen "Error")
      |const::consts -> match const with
        |(s,loc,reper)-> if (s = sexpr) then loc else find sexpr consts
        (* |_-> raise (X_this_should_not_happen "Not matching the const table's form") *)
      (* |_->raise(X_this_should_not_happen "Not matching the const table's form") *)
    in fun sexpr t -> find sexpr t;;
    ...
    let rec run params env = function
      ...
      | ScmVarGet' (Var' (v, Bound (major, minor))) ->
        (Printf.sprintf 
          ...
          minor)
      | ScmIf' (test, dit, dif) -> 
        let if_else = make_if_else () 
        and if_end = make_if_end () in 
        (run params env test)
      ...
      | ScmBox' (Var' (v, Param minor)) -> 
        ...
        ^"\tmov qword [rax], rbx\n"
        ^"\tpop rbx\n"
      | ScmBox' _ -> raise (X_this_should_not_happen "We are not soppussed to get here")
      | ScmBoxGet' var' ->
      ...
      | ScmLambda' (params', Opt opt, body) ->
        let label_loop_env = make_lambda_opt_loop_env ()
         ...
         and label_shrink_end = make_lambda_opt_loop_exit ()
         ...
         ^ "\tcmp r15, 0\n"
         ^ (Printf.sprintf "\tjl %s\n" label_arity_ok)
         ^ "\tmov r14, qword [rsp + 8 * r15]\n"
         ^ "\tmov qword [rsp + 8 * r15], rdx\n"
         ^ "\tmov rdx, r14\n"
         ^ "\tdec r15\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_arity_exact)
         ^ (Printf.sprintf "%s:\n" label_arity_more)
         ...
         ^ (Printf.sprintf "\tjne %s\n" label_arity_more)
         ^ "\tmov r14, 2\n"
         ^ "\tadd r14, qword [rsp + 8 * 2]\n"
         ^ (Printf.sprintf "%s:\n" label_shrink_loop)
         ^ "\tcmp r15, -1\n"
         ^ (Printf.sprintf "\tje %s\n" label_shrink_end)
         ^ "\tmov qword [rsp + 8 * r14], rdx\n"
         ^ "\tmov rdx, qword [rsp + 8 * r15]\n"
         ^ "\tdec r15\n"
         ^ "\tdec r14\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_shrink_loop)
         ^ (Printf.sprintf "%s:\n" label_shrink_end)
       ...
       let applic_code=
          ...
          (List.map
            (fun arg->
                ...
                ^"\tpush rax\n")
            (List.rev args))
        and tc_applic_recycle_frame_loop=make_tc_applic_recycle_frame_loop ()
        and tc_applic_recycle_frame_done=make_tc_applic_recycle_frame_done () in 
        applic_code
        ^(Printf.sprintf "\tpush %d\n" (List.length args))
        ^(run params env proc)
        ^"\tcmp byte [rax], T_closure\n"
        ...
        ^"\tlea rdx, [rbp - 8 * 1]\n"
        ^(Printf.sprintf "%s:\n" tc_applic_recycle_frame_loop)
        ^"\tcmp rcx, 0\n"
        ^(Printf.sprintf "\tje %s\n" tc_applic_recycle_frame_done)
        ^"\tmov rsi, qword [rdx]\n"
        ^"\tmov qword [rbx], rsi\n"
        ^"\tdec rcx\n"
        ^"\tsub rbx, 8 * 1\n"
